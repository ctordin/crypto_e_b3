...
# 2. Funções de Busca (Fundamentos e Gráfico)
...
# 3. Interface Lateral (Gestão de Posição)
with st.sidebar:
...
if btn_analisar:
    with st.spinner("Sincronizando dados e cálculos de risco..."):
        ...
        if df is not None:
            ...
            # BLOCO A: GESTÃO DA SUA CARTEIRA (NOVO!)
            if POSSUO_ACAO and PRECO_COMPRA > 0:
                ...
                if ALVO_ANALISTA > 0:
                    ...
                    m3.metric("Distância do Alvo", f"{distancia_alvo:.1f}%", help="Quanto falta para chegar no alvo do analista")
                
                if lucro_pct > 5:
                    st.success(f"Excelente! Você já está com {lucro_pct:.1f}% de lucro. Hora de proteger a operação.")
                st.markdown("---")
            ...
            if rsi > 65:
                st.warning(f"⚠️ ATENÇÃO: Ativo esticado (RSI {rsi:.1f}). Evite compras novas. Se já tem a ação, considere subir o Stop.")
            ...
            # BLOCO D: GESTÃO DE RISCO (CALCULADA)
            st.subheader("🛡️ Gestão de Risco Estrita")
            v_stop = preco_atual * (1 - STOP_LOSS_PCT)
            
            r1, r2 = st.columns(2)
            r1.metric("Novo Stop Loss (Sair)", f"R$ {v_stop:.2f}", help="Coloque esta ordem na corretora para proteger seu lucro.")
            
            # Se tiver alvo do analista, o "Alvo Trailing" vira o alvo dele
            target = ALVO_ANALISTA if ALVO_ANALISTA > 0 else (preco_atual * 1.10)
            r2.metric("Alvo Estratégico", f"R$ {target:.2f}", "Analista" if ALVO_ANALISTA > 0 else "+10%")
